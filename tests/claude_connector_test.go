...
func TestClaudeConnectorIntegration(t *testing.T) {
	...
	mux.Handle("/.well-known/oauth-authorization-server", middleware.CORS(corsConfig)(http.HandlerFunc(adapter.HandleAuthServerMetadata)))
	...
	t.Run("completes the full OAuth flow when authentication is required", func(t *testing.T) {
		...
		defer func() { _ = resp.Body.Close() }() // Linter Fix
		...
		form := url.Values{"client_id": {"test"}, "csrf_state": {csrfToken}, "api_key": {"test-api-key"}}
		...
		// Linter Fix: Check the error from decoding JSON
		...
		toolReq := map[string]interface{}{"jsonrpc": "2.0", "method": "tools/list", "id": 1}
		...
		if resp.StatusCode != http.StatusOK {
			t.Error("Authenticated call failed")
		}
	})
}
